...
module.exports = {
    ...
    indexFun:function() {
            ...
            $(function () {
                ...
                $.ajax({
                    ...
                    rows.each(function() {
                        ...
                        title.popover({
                            content:`成本价:￥${title.data('popover-cost-price')} | 定价: ￥${title.data('popover-price')} | 最低批发价: ${title.data('popover-source-price')}<br/>30天销量: ${thirtySales} | 90天销量: ${ninetySales} <br/>  ${twoMonth}月份销量: ${twoMonthSales} | ${lastMonth}月份销量: ${lastMonthSales}  <br/>总销量: ${title.data('popover-sales')}`,
                            trigger:'hover'
                        });
                    });
        ...
        {// 改最低批发价
            ...
            $('.link-change-wholesale-price').click(function() {
                let productId = $(this).data('product-id');
                let category2Id=$(this).data('category2-id');
                modalLoading.find('.am-modal-hd').text('正在修改...');
                // console.info(productId);
                modal.modal({
                    ...
                    onConfirm: function(e) {
                        ...
                        if(!$.trim(input.val())) {
                            ...
                            $.ajax({
                                type:'post',
                                url:'/admin/product/sync-wholesale-price',
                                data:{
                                    'productId':productId,
                                    'wholesalePrice':$.trim(input.val()),
                                    'category2Id':category2Id
                                }
                            }).then(
                                result => {
        ...
        {// 批量设置供货商标识号
            ...
            $('.link-change-color-tags').click(function() {
                let productId = $(this).data('product-id');
                let category2Id=$(this).data('category2-id');
                modalLoading.find('.am-modal-hd').text('正在修改...');
                modal.modal({
