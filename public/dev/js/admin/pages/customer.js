...
module.exports = {
        ...
        $('.remove-customer').click(function() {
            ...
            $('#confirm-remove-customer').modal({
                ...
                onConfirm: function(options) {
                    ...
                    $.ajax({
                        type:'post',
                        url:`/order/customer/remove/${item.data('id')}`,
                        success:data => {
                            if(data.success) {
                                location.reload();
                            } else {
                                alert.modal({
                                    relatedTarget: this
                                }).find('.am-modal-bd').html(`无法删除,请先删除该用户所有<a href="/order/order?search-customer-id=${item.data('id')}" target="_blank">发货订单</a>后再进行删除`);
                            }
    ...
    customerTypeAhead:function() {
        ...
        var parentCustomer = new Bloodhound({
            ...
            remote: {
                url:'/order/customer/add/customer-parent',
