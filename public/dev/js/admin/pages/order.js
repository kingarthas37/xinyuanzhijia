...
module.exports = {
    ...
    indexFun: function () {
        ...
        function searchOrder() {
            location.href = utils.urlParamsComponent('/order/order',{
                'limit':$('#limit').val(),
        ...
        //动态输入快递单号
        {
            ...
            shippingCompanyInput.each(function(i) {
                ...
                $(this).change(function(){
                    $.ajax({
                        url:'/order/order/action-tracking-number',
                        data:{
                            orderId,
        ...
        //显示未发货记录数
        {
            ...
            $.ajax({
                url:'/order/order/get-shipping-status-count?preDate=' + date1,
                success(data){
            ...
            $.ajax({
                url:'/order/order/get-shipping-status-count?preDate=' + date3,
                success(data){
                    if(data.mainShopCount ===0) {
                    ...
                    $.ajax({
                        ...
                        success:function(data) {
                            ...
                            if(data.bindingId) {
                                ...
                                $.ajax({
                                    type:'get',
                                    url:'/order/order/get-binding-product',
                                    data: {
                                    ...
                                    <div class="order-split child" data-id="${data.bindingId}" data-stock="${_data.stock}" data-count="${data.bindingNumber * parentProductCount}">
                                        ...
                                        <a class="product-title ${outClassName}" href="/order/order/edit/${$(n).parents('tr').data('order-id')}"><span class="product-count"><strong class="font-red product-count">${data.bindingNumber * parentProductCount}</strong></span>*${_data.name} {id:${data.bindingId}} {库:${_data.stock}}</a>
    ...
    domUpdate: function () {
        ...
        //删除地址
        {
            ...
            addressList.on('click','.remove-address',function() {
                ...
                $.ajax({
                    url:'/order/order/remove-customer-address',
                    data:{
    ...
    customerNameTypeAhead: function () {
        ...
        customerNameInput.typeahead(null, {
            ...
            source: new Bloodhound({
                ...
                remote: {
                    url: '/order/order/add/search-customer',
            ...
            */
            ...
            setTimeout(()=>{
                $.each(productIdArr,function (i,n) {
                    $.ajax({
                        url:`/order/order/product?name=${n}`,
                        async:false,
                        success:(result) => {
    ...
    checkCustomerRename(name,input) {
        ...
        $.ajax({
            url:'/order/order/check-customer-rename',
            data:{name:name}
        }).then(data => {
            if(data.customer) {
