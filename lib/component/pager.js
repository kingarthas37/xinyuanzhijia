...
module.exports = {
    ...
    initHtml(settings) {
        ...
        serializeArr.forEach((key,i) => {
            ...
            let tag = i === serializeArr.length -1 ? '' : '&';
            
            if(pager.prev && key === 'page') {
                serializePrev += `${key}=${pager.prev}${tag}`;
            } else {
                serializePrev += `${key}=${settings.serialize[key]}${tag}`;
            }
            
            if(pager.next && key === 'page') {
                serializeNext += `${key}=${pager.next}${tag}`;
            } else {
                serializeNext += `${key}=${settings.serialize[key]}${tag}`;
            }
        });
