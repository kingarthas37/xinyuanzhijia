...
module.exports = {
    ...
    setShoppingCartCookie(req, res, data, exp, domain) {
        if (domain) {
            res.cookie('xcarts', this.getShoppingCartsDataFormat(data), {maxAge: exp, domain: domain});
        } else {
            res.cookie('xcarts', this.getShoppingCartsDataFormat(data), {maxAge: exp});
        }
    },
    getShoppingCartsDataFormat(data) {
        return this.getEncodeByBase64({'productId':data.productId,'count':data.count});
    }
};
