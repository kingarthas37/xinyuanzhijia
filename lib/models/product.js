...
module.exports = {
    ...
    getProducts(options, isCount){
        ...
        let order = '';
        if (!options.select) {
            options.select = 'objectId,isHandmade,imageSource,tags,reserve,isHot,sales,originalPrice,wholesalePrice,category1,category2,name,productId,nameTb,nameEn,price,isOnsale,mainImage,review,pageViews,isOnly,stock,isRefund,productMethod,purchaseLink,shopLink,groups,approval,onsaleDate,isShortStock,isUpdateStock,updateStockDate, offlineTag, costPrice,sold,isTranslation,parentProductId,isParent,isSales,isStock,discount,maximum,colorTag,isPublic,detail,property,comment,bindingId,bindingNumber'
        }
        ...
        if (options.searchComment) {
            cql += " and comment regexp '(?i)"+options.searchComment+"'";
        }
        ...
        if (options.isSales) {
            cql += ' and (isSales = 1 or isSales = 2 or isSales = 3 or isSales = 4 or isSales = 5)';
        ...
        if (options.colorTag) {
            cql += ' and colorTag = ?';
            pvalues.push(options.colorTag);
        }
        if (options.isPublic) {
            cql += ' and isPublic = ?';
            pvalues.push(options.isPublic);
        }
        if (!isCount) {
            if (!options.parentProductId) {
                if (options.price) {
