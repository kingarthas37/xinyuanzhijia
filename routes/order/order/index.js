    ...
    query.find().then(images => {
        ...
        images.forEach((image, i) => {
            ...
            if(JSON.stringify(image.get('mainImage'))=== '{}') {
                imgArr.push({
                    url:'//order-image.wish698.cn/d4c225054f005a4d7d39.png'
                });
            } else {
            ...
            data[image.get('productId')] = imgArr[0].url.replace('ac-QuiPuWpJ.clouddn.com','order-image.wish698.cn').replace('ac-JoaBcRTt.clouddn.com','order-image.wish698.cn').replace(/lc-QuiPuWpJ.cn-n1.lcfile.com/gi, 'order-image.wish698.cn');
        });
    ...
    pro.getProducts(options, false).then(results => {
        ...
        for (let i = 0; i < results.length; i++) {
            ...
            for (let key in results[i].get('mainImage')) {
                imageArr.push(results[i].get('mainImage')[key].url.replace('ac-QuiPuWpJ.clouddn.com','order-image.wish698.cn').replace(/lc-QuiPuWpJ.cn-n1.lcfile.com/gi, 'order-image.wish698.cn') );
            }
            let obj = {
